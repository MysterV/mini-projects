...
if len(sys.argv) < 2:
    ...
    print('1. fps (default 30): specify GIF framerate')
    print('2. loop: 0 -> infinite, -1 -> no, 1+ -> specify loop count (default 0)')
    print('3. width: -1 -> original size, 1+ -> specify GIF width (default 640)')
    print('4. start: specify timestamp or time in seconds in the video to start at (default 0)')
    print('5. duration: duration of the GIF in seconds (default - calculated as (video length-start))')
    print(f'\n\nUsage example: navigate to the folder of the video you want to convert and run:')
    print(f'\nExample1 (default parameters): {__file__} video.mkv')
    print(f'Example2 (custom width): {__file__} video.mkv 30 0 1920')
    print(f'Example3 (convert only a fragment): {__file__} video.mkv 30 0 -1 00:05:43 30')
    input('\nPress Enter to exit...')
    sys.exit(1)
...
ffmpeg_command = (
    # f'ffmpeg -ss {start_time} -t {duration} -i "{input_file}" -vf "fps={fps},scale={width}:-1:flags=lanczos,split[s0][s1];[s0]palettegen=stats_mode=diff[p];[s1][p]paletteuse" -loop {loop} "{output_file}"'
